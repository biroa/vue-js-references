...
/**
 * @description Calculate the full price on call
 * @returns {string}
 */
const totalCalculator = ()=>{
  let total = products.value.reduce((acc, product) => {
    //acc + product.price
...
watch(products.value, ([newVal, oldVal]) => {
  if(newVal != oldVal){
    // call the calculator on change
    total.value = totalCalculator();
  }
...
onMounted(()=>{
  // Init the price on the first page load
  total.value = totalCalculator();
})
